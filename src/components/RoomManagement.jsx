...
function RoomManagement() {
    ...
    useEffect(() => {
        ...
        const firstPromise = fetch("http://localhost:3000/analytics/room-overview", {
            method: "GET"
        }).then((response) => {
            return response.json();
        });
        // get initial room list
        const secondPromise = fetch("http://localhost:3000/room", {
            method: "GET"
        }).then((response) => {
            return response.json();
        });
    ...
    async function fetchRoomOverview() {
        const response = await fetch("http://localhost:3000/analytics/room-overview");
        const data = await response.json();
        ...
        <section className="room-management">
            <div className="room-filters">
                ...
                <div className="filter-option">
                    <input checked={inputStates.occupied} type="radio" id="occupied" onChange={() => {
                ...
                <div className="filter-option">
                    <input checked={inputStates.vacant} type="radio" id="vacant" value="0" onChange={() => {
                ...
                <h5>Room Price</h5>
                <div className="filter-option">
                    <input checked={inputStates.single} type="radio" name="price" id="single-price" value="2700" onChange={() => {
                ...
                <div className="filter-option">
                    <input checked={inputStates.double} type="radio" name="price" id="double-price" value="3500" onChange={() => {
            ...
            <div className="room-list">
                <RoomPopUp
                    fetchRoomList={fetchRoomList} fetchRoomOverview={fetchRoomOverview}
                    popUpType={popUpType} roomInfo={selectedRoom.current}
                    togglePopUp={togglePopUp} showPopUp={showPopUp}
                >
                </RoomPopUp>
                <h3>ROOM LIST</h3>
                <div className="card-container">
                    {
                        roomList.map((element) => {
                            return <RoomCard
                                removeTenant={removeTenant} togglePopUp={togglePopUp}
                                popUpType={togglePopUpType} onAssign={onAssign}
                                key={element.room_number} roomImage={[singleBed, doubleBed]}
                                roomInfo={element}
                            >
                            </RoomCard>;
                        })
